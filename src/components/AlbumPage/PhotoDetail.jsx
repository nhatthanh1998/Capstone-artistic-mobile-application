...
export const PhotoDetail = ({photo, isVisible, handlePressBack}) => {
    ...
    useEffect(() => {
        if(photo != null) {
            Image.getSize(accessURL, (_, height) => {
    ...
    if(photo === null) {
        ...
        return (
            <Modal style={tailwind("m-0")} isVisible={isVisible}>
                <View style={tailwind("w-full h-full bg-gray-900")}>
                    <View style={tailwind("flex flex-row z-20 absolute py-5 w-full")}>
                        ...
                        <View style={tailwind("w-1/3")}>
                            <Text style={tailwind("text-sm font-thin text-white text-center")}>{photoName}</Text>
                        </View>
